    ...
    def __call__(self, batch):
        ...
        #     batch_label.append(sample['label'])
            ...
            # if self.config.text_encoder.type == "bert":
                ...
                # batch_token_type_ids.append(sample['token_type_ids'][0])
                # batch_attention_mask.append(sample['attention_mask'][0])
        batch_label = [sample['label_id'] for sample in batch]
        batch_input_ids = torch.cat([torch.tensor(sample['input_ids']).squeeze().unsqueeze(0) for sample in batch], dim=0)
        batch_token_type_ids = torch.cat([torch.tensor(sample['token_type_ids']).squeeze().unsqueeze(0) for sample in batch], dim=0)
        batch_attention_mask = torch.cat([torch.tensor(sample['attention_mask']).squeeze().unsqueeze(0) for sample in batch], dim=0)
        ...
        # print([batch_input_ids[i].shape for i in range(len(batch_input_ids))])
